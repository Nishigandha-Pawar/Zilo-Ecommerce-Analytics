...
SELECT 
    p.category, 
    SUM(o.quantity * p.price_sale) AS total_revenue
FROM 
    orders_clean o
JOIN 
...
SELECT 
    ROUND(SUM(o.quantity * p.price_sale) / COUNT(DISTINCT o.order_id),2) AS AOV
FROM
    orders_clean o
        JOIN
    products_clean p ON o.product_id = p.product_id;
...
SELECT 
    p.product_name, SUM(o.quantity * p.price_sale) AS revenue
FROM
    orders_clean o
        JOIN
    products_clean p ON o.product_id = p.product_id
...
SELECT 
    ...
    SUM(o.quantity * p.price_sale) AS monthly_revenue
FROM
    orders_clean o
        JOIN
    products_clean p ON o.product_id = p.product_id
...
SELECT 
    p.brand, SUM(o.quantity * p.price_sale) AS revenue
FROM
    orders_clean o
        JOIN
    products_clean p ON o.product_id = p.product_id
